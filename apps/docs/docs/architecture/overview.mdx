...
```mermaid
flowchart TB
    subgraph Client["Application Client (Next.js)"]
        Pages["Pages SSR<br/>/products, /products/new, /products/:id"]
        ServerActions["Server Actions<br/>createProduct, updateProduct, registerMedia"]
        ApiClient["API Client<br/>(@maison-amane/api)"]
    end
    ...
    ApiClient --> Handler
    Pages -->|"upload direct"| CloudinaryCDN
    Handler --> AppHandler
    ...
    Exchange -->|"product.created<br/>product.updated"| MediaQueue
    ...
    class Shopify,CloudinaryCDN external
    class Pages,ServerActions,ApiClient client
```
...
**Flux** : `RabbitMQ Event → Consumer (driving) → Application Handler → Repository (driven)`
  ...
  <TabItem value="client" label="Application Client">

L'application back-office **Next.js 16 + MUI 7** permet la gestion des produits depuis un navigateur.

| Composant | Rôle | Route |
|-----------|------|-------|
| `ProductListGrid` | Grille de cards produits | `/products` |
| `ProductDetailShell` | Layout détail produit + context provider | `/products/new`, `/products/:id` |
| `ProductEditorContent` | Formulaire + drag & drop images | (enfant de `ProductDetailShell`) |
| `ProductFormContext` | State management : mode, images, titre, sauvegarde | — |
| `useImageUpload` | Upload Cloudinary + enregistrement media | — |

Voir [Architecture Client](./client/overview) pour les détails complets.
