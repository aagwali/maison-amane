...
const formatEnumParseError = (type: Record<string, string | number>) => (value: ParseIssue) =>
  `Expected one of: ${Object.values(type).join(' | ')}, actual "${value.actual}"`

export const createEnumSchema = <T extends Record<string, string | number>>(enumObject: T) =>
